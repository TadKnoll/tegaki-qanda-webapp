...
- 採点後、**正しく書けたと思うマスを自由に保存**できる（`localStorage`）
  - 「◯」判定の字: 「◯の手書き N 字を学習用に保存」ボタンでまとめて保存
  - 「△」「×」でも正しく書けていれば: 「マスN『X』は正しく書けた → 学習用に保存」ボタンで保存
  - 自己採点で「○ あってる」を選んだ字も自動保存
- 画面下の「ダウンロード」で JSON を書き出せる
  （例: `kanji_training_data_20260811_1230.json`）
- ためた JSON を PC に渡し、`training/train_cnn.py` に `REALDATA=<パス>` で指定して再学習すると、
  実際の手書きに合わせて認識が改善する（詳細は `training/README.md` 参照）
...
| `c028a6e` | 学習 14 エポック完了 + ブラウザ統合（onnxruntime-web 自己ホスト・採点を CNN に切替） |
| `ecd89c8` | バイナリ（wasm/onnx/pt）の改行変換防止を明示 |
| （未コミット） | 採点フリーズ修正（推論の直列化）+ 学習データ収集機能 + `REALDATA` 再学習対応 |
